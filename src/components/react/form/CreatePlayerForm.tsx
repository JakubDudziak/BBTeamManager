...
import {newPlayerSchema, type newPlayerFormValuesOutput, type newPlayerFormValuesInput} from "../../schemas/newPlayer.ts"
import {useForm} from "react-hook-form";
import {zodResolver} from "@hookform/resolvers/zod";
import React from "react";

export default function CreatePlayerForm() {
    const {register, handleSubmit, formState: { errors }} = useForm<newPlayerFormValuesInput, any, newPlayerFormValuesOutput>({
        resolver: zodResolver(newPlayerSchema),
        mode: "onChange",
        reValidateMode: "onChange"
    });

    const onSubmit = (data: newPlayerFormValuesOutput) => {
        ...
        <div className="flex flex-col m-4 p-1 bg-white rounded-xl shadow-lg">
            ...
            <form className="flex flex-col items-center m-4" onSubmit={
                ...
                <div className="w-full flex flex-col items-start">
                    ...
                    {
                        errors.firstName ?
                            <p className="h-12 text-red-500">{errors.firstName.message}</p> :
                            <div className="h-6"/>
                    }
                    ...
                    {
                        errors.lastName ?
                            <p className="h-12 text-red-500">{errors.lastName.message}</p> :
                            <div className="h-6"/>
                    }
                    <label htmlFor="number" className="text-xl pb-1">Number</label>
                    <input id="number"  {...register("number")} type="number" placeholder="Set number from 0 to 99" className="text-base w-full border-solid border-b-2 border-(--middle-gray)"/>
                    {
                        errors.number ?
                            <p className="h-12 text-red-500">{errors.number.message}</p> :
                            <div className="h-6"/>
                    }
                    <p>{}</p>
                    ...
                    {
                        errors.position ?
                            <p className="h-12 text-red-500">{errors.position.message}</p> :
                            <div className="h-6"/>
                    }
                    ...
                    {
                        errors.heightCM ?
                            <p className="h-12 text-red-500">{errors.heightCM.message}</p> :
                            <div className="h-6"/>
                    }
                    ...
                    {
                        errors.weightKG ?
                            <p className="h-12 text-red-500">{errors.weightKG.message}</p> :
                            <div className="h-6"/>
                    }
                    ...
                    {
                        errors.age ?
                            <p className="h-12 text-red-500">{errors.age.message}</p> :
                            <div className="h-6"/>
                    }
                    ...
                    {
                        errors.photoURL ?
                            <p className="h-12 text-red-500">{errors.photoURL.message}</p> :
                            <div className="h-6"/>
                    }
                </div>
