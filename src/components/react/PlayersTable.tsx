...
export default function PlayersTable({ players, buttonFnOnOpen }: { players: Player[], buttonFnOnOpen: (modalType: Modals, playerId: number) => void }) {
    return (
                            ...
                            return (
                                <tr key={player.id} className={"group bg-white even:bg-(--light-gray) border-b-1 border-(--middle-gray)"}>
                                    ...
                                    <td className={"p-6 align-middle text-left select-none"}>
                                        <button className={"bg-[url(public/icons/eye-icon.png)] size-[50px] border-none bg-white bg-size-[70%] bg-no-repeat bg-center group-even:bg-(--light-gray)"} type={"button"} onClick={() => buttonFnOnOpen(Modals.ViewPlayer, player.id)}></button>
                                        <button className={"bg-[url(public/icons/edit-icon.png)] size-[50px] border-none bg-white bg-size-[70%] bg-no-repeat bg-center group-even:bg-(--light-gray)"} type={"button"} onClick={() => buttonFnOnOpen(Modals.EditPlayer, player.id)}></button>
                                        <button className={"bg-[url(public/icons/trash-icon.png)] size-[50px] border-none bg-white bg-size-[70%] bg-no-repeat bg-center group-even:bg-(--light-gray)"} type={"button"} onClick={() => buttonFnOnOpen(Modals.RemovePlayer, player.id)}></button>
                                    </td>
                                </tr>
                            )
                        })
